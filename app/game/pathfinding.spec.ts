...
describe('pathfinding', () => {
    it('水タイルを迂回する', () => {
        ...
        const path = findPath(grid, 0, 0, 4 * TILE_PX, 0, 1)
    ...
    it('Rank3 は水を渡れる', () => {
        ...
        expect(getTileCost(grid, 1, 0, 3)).toBe(2)
        expect(findPath(grid, 0, 0, TILE_PX, 0, 3)).toEqual([
            { x: 0, y: 0 },
            { x: TILE_PX, y: 0 },
        ])
    })
    ...
    it('右端列付近でもキー衝突せずに経路を復元する', () => {
        ...
        const path = findPath(grid, 50 * TILE_PX, 10 * TILE_PX, 52 * TILE_PX, 10 * TILE_PX, 1)
