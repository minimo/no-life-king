...
output_path = os.path.join(output_dir, 'grid32x16_magnified.png')
...
for y in range(rows + 1):
    draw.line((0, offset_y + y * grid_h, img.width, offset_y + y * grid_h), fill=(0, 255, 0, 200), width=3)
for x in range(cols + 1):
    draw.line((offset_x + x * grid_w, 0, offset_x + x * grid_w, img.height), fill=(0, 255, 0, 200), width=3)

for y in range(rows):
    for x in range(cols):
        # Determine if empty from original unscaled image
        orig_box = (16 + x*32, 16 + y*16, 16 + (x+1)*32, 16 + (y+1)*16)
        crop = orig_img.crop(orig_box)
        if crop.getbbox():
            px, py = offset_x + x * grid_w + 4, offset_y + y * grid_h + 4
            text = f"{x},{y}"
            if font:
